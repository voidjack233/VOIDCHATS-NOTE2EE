...
interface FriendRequestRow extends Record<string, unknown> {
...
function mapFriendRequest(row: FriendRequestRow) {
  return {
    ...row,
...
async function getOutgoingRequests(userId: string) {
  const result = await pool.query(
    `SELECT
       f.id as friendship_id,
       f.created_at,
