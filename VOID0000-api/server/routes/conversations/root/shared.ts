...
export interface ConversationMemberRow extends Record<string, unknown> {
  user_id: string;
  role?: string;
...
export interface ConversationRow extends Record<string, unknown> {
  ...
  dm_display_name?: unknown;
  dm_username?: unknown;
}

export const buildObjectUrl = (bucket: string, filename: string | null | undefined) => (
  filename ? `${baseUrl}/${bucket}/${filename}` : null
);

export const buildAvatarUrl = (filename: string | null | undefined) => (
  filename ? `${baseUrl}/${BUCKET}/${filename}` : null
);

export const resolveGroupIconBucket = (filename: string) => (
  filename?.startsWith('groups/')
    ? BUCKET
    : GROUP_AVATAR_BUCKET
);

export const buildGroupIconUrl = (filename: string | null | undefined) => (
...
export function normalizeConversationRow<T extends ConversationRow>(conv: T) {
  return {
    ...conv,
    ...
    first_message_at: conv.first_message_at
      ? new Date(
        conv.first_message_at instanceof Date
          ? conv.first_message_at.getTime()
          : String(conv.first_message_at),
      ).toISOString()
      : null,
    member_count: conv.member_count != null ? parseInt(String(conv.member_count), 10) : 0,
    unread_count: conv.unread_count != null ? parseInt(String(conv.unread_count), 10) : 0,
    slowmode_seconds: conv.slowmode_seconds != null ? parseInt(String(conv.slowmode_seconds), 10) : 0,
