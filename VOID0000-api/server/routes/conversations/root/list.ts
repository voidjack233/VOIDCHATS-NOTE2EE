...
function normalizePreviewMessageRow(row: unknown): PreviewMessageRow | null {
  if (!row || typeof row !== 'object') return null;

  return {
    message_id: Reflect.get(row, 'message_id'),
    sender_id: Reflect.get(row, 'sender_id'),
    content: Reflect.get(row, 'content'),
    attachments: Reflect.get(row, 'attachments'),
    is_deleted: Reflect.get(row, 'is_deleted'),
  };
}

interface ConversationListRow extends Record<string, unknown> {
  id: string;
  storage_conversation_id?: string | null;
}

function buildLastMessagePreview(
  messageRow: PreviewMessageRow | null,
  ...
  const attachmentCount = Array.isArray(messageRow.attachments)
    ...
    : 0;
  const isSender = messageRow.sender_id != null && String(messageRow.sender_id) === currentUserId;
...
async function hydrateConversationPreview(
  ...
  try {
    ...
    const latestVisible = normalizePreviewMessageRow(result.rows[0]);

    return {
      ...row,
      last_message_id: latestVisible?.message_id != null ? String(latestVisible.message_id) : null,
      last_message_sender_id: latestVisible?.sender_id != null ? String(latestVisible.sender_id) : null,
...
export async function getUserConversations(userId: string) {
  ...
  return Promise.all(
    result.rows.map(async (row) => {
      ...
      const conversationRow = { ...hydrated };
      delete conversationRow.storage_conversation_id;
