...
router.put('/:conversationId/icon', async (req, res) => {
  ...
  try {
    ...
    if ((metadata.width ?? 0) > MAX_ICON_DIMENSION || (metadata.height ?? 0) > MAX_ICON_DIMENSION) {
    ...
    if (resolvedConversation.icon_filename && resolvedConversation.icon_filename !== iconFilename) {
      try {
        ...
        console.warn('Could not delete old group icon:', message);
      }
    }
  ...
  try {
    ...
    if (resolvedConversation.icon_filename) {
      try {
        ...
        const message = err instanceof Error ? err.message : String(err);
        console.warn('Could not delete group icon:', message);
      }
    }
