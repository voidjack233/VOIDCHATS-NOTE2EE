...
type ConversationMemberResolution =
  | {
...
function getConversationDownloadIdentifier(
  conversation: Pick<ConversationIdentityRow, 'id' | 'public_id'>,
): string {
  return conversation.public_id ? String(conversation.public_id) : String(conversation.id);
}

function buildPrivateAttachmentUrl(
  conversation: Pick<ConversationIdentityRow, 'id' | 'public_id'>,
  attachmentId: string,
): string {
...
async function resolveConversationForMember(
  conversationIdentifier: unknown,
  userId: string,
): Promise<ConversationMemberResolution> {
  ...
  const member = await pool.query<ConversationMemberRow>(
...
async function findAttachmentObject(
  ...
  const result = await sentinel.guard(
    attachmentFlightKey,
    () => pool.query<AttachmentObjectRow>(
...
function statAttachmentObject(objectKey: string): Promise<BucketItemStat> {
...
async function readAttachmentObject(objectKey: string): Promise<Buffer> {
...
async function readAttachmentObjectWithinBudget(
  objectKey: string,
  objectSize: number,
): Promise<Buffer | null> {
  if (
...
router.post<{ conversationId: string }>('/', attachmentUploadLimiter, async (req, res) => {
  ...
  let conversation: ConversationIdentityRow;
...
router.delete<{ conversationId: string; attachmentId: string }>('/:attachmentId', async (req, res) => {
  const userId = req.user?.id;
