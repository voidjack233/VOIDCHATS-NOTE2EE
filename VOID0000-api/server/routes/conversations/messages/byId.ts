...
type ContextDirection = 'older' | 'newer';

interface FetchVisibleContextOptions {
  storageConversationId: string;
  conversationPublic: string | null;
  cursor: cassandra.types.TimeUuid;
  direction: ContextDirection;
  limit: number;
}

interface VisibleContextSide {
  messages: MappedStoredMessage[];
  hasMore: boolean;
}

function clampContextLimit(value: unknown): number {
...
function compareMessagesByCreatedAtAsc(
  left: MappedStoredMessage,
  right: MappedStoredMessage,
): number {
  const timeDiff = new Date(String(left.created_at)).getTime() -
...
async function fetchVisibleContextSide({
  ...
  direction,
  limit,
}: FetchVisibleContextOptions): Promise<VisibleContextSide> {
  if (limit <= 0) {
    return { messages: [], hasMore: false };
  }

  const conversationUuid = cassandra.types.Uuid.fromString(storageConversationId);
  const collectedMessages: MappedStoredMessage[] = [];
  ...
  while (collectedMessages.length <= limit && !exhausted && iterations < maxFetchIterations) {
    ...
    const lastMessageId = lastRow && typeof lastRow === 'object'
      ? Reflect.get(lastRow, 'message_id')
      : undefined;
    if (!(lastMessageId instanceof cassandra.types.TimeUuid)) {
      exhausted = true;
    } else {
...
router.get<{ conversationId: string; messageId: string }>('/:messageId/context', async (req, res) => {
  ...
  try {
    ...
    const uniqueMessagesById = new Map<string, MappedStoredMessage>();
...
router.get<{ conversationId: string; messageId: string }>('/:messageId', async (req, res) => {
  const userId = req.user?.id;
  const { conversationId: conversationIdentifier, messageId } = req.params;
...
router.patch<{ conversationId: string; messageId: string }>('/:messageId/preview', async (req, res) => {
  const userId = req.user?.id;
  const { conversationId: conversationIdentifier, messageId } = req.params;
...
router.put<{ conversationId: string; messageId: string }>('/:messageId', async (req, res) => {
  const userId = req.user?.id;
  const { conversationId: conversationIdentifier, messageId } = req.params;
  ...
  try {
    ...
    if (!storedAttachments.every((attachment): attachment is string => typeof attachment === 'string')) {
      throw new Error('Stored message attachments are malformed');
    }
...
router.delete<{ conversationId: string; messageId: string }>('/:messageId', async (req, res) => {
  const userId = req.user?.id;
  const { conversationId: conversationIdentifier, messageId } = req.params;
