...
async function broadcastMembershipUpdate(
...
router.delete<{ conversationId: string }>('/@me', async (req, res) => {
...
router.delete<{ conversationId: string; targetUserId: string }>('/:targetUserId', async (req, res) => {
  const actorUserId = req.user?.id;
