...
interface MembershipAccessError {
  error: {
    status: number;
    body: { error: string };
  };
...
async function ensureConversationMember(
  ...
  userId: string,
): Promise<MembershipAccess | MembershipAccessError> {
...
async function emitGroupMembershipUpdate(
  conversation: ConversationIdentityRow,
): Promise<void> {
...
router.get<{ conversationId: string }>('/', async (req, res) => {
  ...
  try {
    const access = await ensureConversationMember(pool, req.params.conversationId, userId);
    if ('error' in access) {
...
router.post<{ conversationId: string }>('/', async (req, res) => {
  ...
  try {
    ...
    const access = await ensureConversationMember(client, req.params.conversationId, userId);
    if ('error' in access) {
...
router.post<{ conversationId: string; requestId: string }>('/requests/:requestId/approve', async (req, res) => {
  ...
  try {
    ...
    const access = await ensureConversationMember(client, req.params.conversationId, actorUserId);
    if ('error' in access) {
      await client.query('ROLLBACK');
      return res.status(access.error.status).json(access.error.body);
    }
    ...
    await emitGroupMembershipUpdate(conversation).catch((emitErr) => {
...
router.post<{ conversationId: string; requestId: string }>('/requests/:requestId/decline', async (req, res) => {
  const actorUserId = req.user?.id;
  const requestId = parseInt(req.params.requestId, 10);
  ...
  try {
    const access = await ensureConversationMember(pool, req.params.conversationId, actorUserId);
    if ('error' in access) {
      return res.status(access.error.status).json(access.error.body);
    }
    const { conversation, membership } = access;
...
router.post<{ conversationId: string; inviteId: string }>('/:inviteId/revoke', async (req, res) => {
  const actorUserId = req.user?.id;
