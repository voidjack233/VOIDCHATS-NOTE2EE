...
export interface InviteLinkPreview extends Record<string, unknown> {
  id: number;
  code: string;
  max_uses: unknown;
  use_count: unknown;
  ...
  conversation_icon_filename: string | null;
...
function isInviteLinkActive(inviteLink: InviteLinkPreview | null): boolean {
  if (!inviteLink || inviteLink.is_revoked) return false;

  if (
    inviteLink.expires_at &&
    new Date(
      inviteLink.expires_at instanceof Date
        ? inviteLink.expires_at.getTime()
        : String(inviteLink.expires_at),
    ).getTime() <= Date.now()
  ) {
    return false;
  }
...
const buildObjectUrl = (bucket: string, filename: string | null | undefined) => (
  filename ? `${baseUrl}/${bucket}/${filename}` : null
);
const resolveGroupIconBucket = (filename: string) => (
  filename?.startsWith('groups/')
    ? BUCKET
    : GROUP_AVATAR_BUCKET
);
const buildGroupIconUrl = (filename: string | null | undefined) => (
  filename ? buildObjectUrl(resolveGroupIconBucket(filename), filename) : null
);

async function getInviteLinkPreview(
  code: string,
  db: DatabaseQueryable = pool,
): Promise<InviteLinkPreview | null> {
  const result = await db.query<InviteLinkPreview>(
    `SELECT
       cil.id,
       cil.code,
       cil.max_uses,
...
router.get<{ code: string }>('/:code/status', authenticateUser, async (req, res) => {
...
router.post<{ code: string }>('/:code/request', authenticateUser, async (req, res) => {
  const userId = req.user?.id;
