...
function getNormalizedHostname(parsedUrl: URL): string {
...
function getRequestPort(parsedUrl: URL): number {
...
async function assertPublicHttpUrl(parsedUrl: URL): Promise<{
  ...
  const resolvedFamily = net.isIP(addresses[0].address);
  return {
    address: addresses[0].address,
    family: resolvedFamily === 4 || resolvedFamily === 6 ? resolvedFamily : undefined,
    hostname,
...
function destroyResponseBody(response: PinnedResponse): void {
  response.body.destroy();
}

async function fetchPinnedResponse(
  url: URL,
  options: PinnedRequestInput = {},
...
async function fetchJsonWithLimit(url: URL): Promise<unknown> {
  try {
    ...
    const parsed: unknown = JSON.parse(new TextDecoder('utf-8').decode(buffer));
    return parsed;
  } catch {
    return null;
  }
...
async function fetchYoutubePreview(url: URL): Promise<LinkPreview | null> {
  ...
  const thumbnail = await normalizePublicResourceUrl(
    Reflect.get(data, 'thumbnail_url'),
    url,
  );
  const title = normalizeText(Reflect.get(data, 'title'));
  if (!title && !thumbnail) {
    return null;
  }
...
function getMetaContent(html: string, keys: readonly string[]): string | null {
...
function getHostnameLabel(url: URL): string {
...
function getFallbackDescription(url: URL): string {
...
function createScrapeFailureFallbackPreview(
  url: URL,
...
async function firstPublicResourceUrl(
...
function getLinkHrefByRel(
  html: string,
  wantedRels: readonly string[],
): string | null {
...
function getVideoPoster(html: string): string | null {
  ...
  if (typeof value === 'object') {
    for (const key of ['thumbnailUrl', 'thumbnail', 'image', 'url', 'contentUrl']) {
      if (Object.prototype.hasOwnProperty.call(value, key)) {
        collectJsonLdImageCandidates(Reflect.get(value, key), output);
        if (output.length >= 10) break;
      }
    }
  }
...
function getJsonLdImageCandidates(html: string): string[] {
...
async function normalizePublicResourceUrl(
  value: unknown,
  baseUrl: URL,
): Promise<string | null> {
...
async function extractPreview(html: string, finalUrl: URL): Promise<LinkPreview> {
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const status = Number(getErrorProperty(error, 'status')) ||
      (getErrorProperty(error, 'name') === 'AbortError' ? 504 : 500);
    if (status !== 400) {
      try {
        const fallbackUrl = parseCandidateUrl(getErrorProperty(error, 'finalUrl')) || parsedUrl;
        await assertPublicHttpUrl(fallbackUrl);
        const fallbackPreview = createScrapeFailureFallbackPreview(fallbackUrl, {
          contentType: getErrorProperty(error, 'contentType'),
        });
        res.set('Cache-Control', 'private, max-age=300');
    ...
    return res.status(status).json({
      success: false,
      error: status >= 500
        ? 'Unable to fetch link preview right now.'
