...
interface NormalizedRateLimitDimension {
...
interface NormalizedAuthLockoutPolicy {
  keyPrefix: string;
  dimensions: NormalizedRateLimitDimension[];
...
function normalizePolicy(policy: RateLimitPolicy): NormalizedAuthLockoutPolicy {
...
function hashKeyPart(value: unknown): string {
  return crypto.createHash('sha256').update(String(value)).digest('hex').slice(0, 40);
}

function getSubject(req: Request, fields: readonly string[]): string | null {
...
function getDimensionId(
  req: Request,
  res: Response,
  policy: NormalizedAuthLockoutPolicy,
  scope: RateLimitScope,
): string | null {
...
export function createAuthLockoutLimiter(rawPolicy: RateLimitPolicy): RequestHandler {
