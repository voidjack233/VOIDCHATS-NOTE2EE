...
interface TrustScoreRow extends QueryResultRow {
  ...
  last_account_created_at: string | Date | null;
}

type TrustScoreResult =
  | { score: number; isNew: true; deviceId: string | null; record?: never }
  | { score: number; isNew: false; deviceId: string; record: TrustScoreRow };

type TrustEvent = 'LOGIN_SUCCESS' | 'LOGIN_FAILED' | 'CAPTCHA_PASSED' | 'CAPTCHA_FAILED';

export async function getTrustScore(req: Request): Promise<TrustScoreResult> {
  try {
    ...
    return {
      score: Number.parseFloat(String(record.trust_score)),
      isNew: false,
      deviceId,
      record
    };
...
export async function shouldRequireCaptchaForRegistration(req: Request): Promise<boolean> {
  try {
