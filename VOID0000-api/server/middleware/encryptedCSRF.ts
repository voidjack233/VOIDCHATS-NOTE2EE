...
export interface EncryptedCsrfTokenPair {
  plainToken: string;
  encryptedToken: string;
}

export const generateEncryptedCSRFToken = (): EncryptedCsrfTokenPair => {
...
export const verifyEncryptedCSRFToken = (encryptedToken: unknown): string | null => {
  try {
...
export const encryptedCSRFProtection: RequestHandler = (req, res, next) => {
  ...
  if (typeof csrfTokenHeader !== 'string' || decryptedToken !== csrfTokenHeader) {
    return res.status(403).json({
      success: false,
      message: 'CSRF token validation failed',
