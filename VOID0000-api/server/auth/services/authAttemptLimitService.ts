...
import type {
  SecurityCounterClient,
  SecurityCounterState,
} from './securityCounterService.js';
...
export class SecurityLimitExceededError extends Error {
  ...
  readonly state: SecurityCounterState;

  constructor(code: string, state: SecurityCounterState) {
...
function hashKeyPart(...parts: unknown[]): string {
  return crypto
    .createHash('sha256')
...
function getRequestSource(req: Request): { ip: string; device: string } {
  return {
...
interface ClearEmailVerificationOptions extends EmailVerificationOptions {
  includeIp?: boolean;
}
...
function getEmailVerificationKeys({
  req,
  tokenHash,
  userId,
...
function getSensitiveActionKeys({
  req,
  userId,
  action,
}: SensitiveActionOptions): string[] {
...
function mergeCounterStates(
  states: SecurityCounterState[],
): SecurityCounterState {
...
async function readEmailVerificationState({
  ...
  userId,
  client,
}: EmailVerificationOptions): Promise<SecurityCounterState> {
...
export async function assertEmailVerificationAllowed(
  options: EmailVerificationOptions,
): Promise<SecurityCounterState> {
...
export async function clearEmailVerificationFailures({
  ...
  client,
  includeIp = false,
}: ClearEmailVerificationOptions): Promise<void> {
  const keys = getEmailVerificationKeys({ req, tokenHash, userId });
...
export async function assertSensitiveTwoFactorActionAllowed({
  ...
  action,
  client,
}: SensitiveActionOptions): Promise<SecurityCounterState> {
...
export function clearSensitiveTwoFactorActionFailures({
  ...
  action,
  client,
}: SensitiveActionOptions): Promise<void> {
  return clearFixedWindowCounters(
