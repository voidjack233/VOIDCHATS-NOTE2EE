...
interface EncryptedReceiptToken {
  iv: string;
  ciphertext: string;
  authTag: string;
}

interface StoredRotationReceipt extends EncryptedReceiptToken {
  version: number;
  userId: string;
  deviceId: string;
  consumedJti: string;
...
interface StoreReceiptOptions {
  consumedTokenHash: unknown;
  consumedJti: unknown;
  userId: unknown;
  deviceId: unknown;
  replacementTokenHash: unknown;
  replacementJti: unknown;
...
interface DeleteReceiptOptions {
  consumedTokenHash: string | null | undefined;
  replacementTokenHash: string | null | undefined;
}

export interface RefreshRotationReceipt {
  replacementRefreshToken: string;
  replacementTokenHash: string;
  replacementJti: string;
}

function isStoredRotationReceipt(value: unknown): value is StoredRotationReceipt {
  return (
    typeof value === 'object' &&
    value !== null &&
    'version' in value &&
    ...
    'consumedJti' in value &&
    typeof value.consumedJti === 'string' &&
    'replacementTokenHash' in value &&
    typeof value.replacementTokenHash === 'string' &&
    'replacementJti' in value &&
    typeof value.replacementJti === 'string' &&
    'expiresAt' in value &&
...
function getLegacyReceiptKey(consumedTokenHash: unknown): string {
  return `${LEGACY_RECEIPT_KEY_PREFIX}${consumedTokenHash}`;
}

function getReceiptEncryptionKey(): Buffer {
...
function encryptReplacementToken(
  replacementRefreshToken: string,
...
function decryptReplacementToken(
  receipt: EncryptedReceiptToken,
  metadata: ReceiptMetadata,
): string {
...
}: StoreReceiptOptions): Promise<boolean> {
  try {
...
export async function getRefreshRotationReceipt({
  ...
  deviceId,
  replacementTokenHash,
}: GetReceiptOptions): Promise<RefreshRotationReceipt | null> {
  try {
    ...
    const parsedReceipt: unknown = JSON.parse(rawReceipt);
    if (!isStoredRotationReceipt(parsedReceipt)) {
      return null;
    }
...
export async function deleteRefreshRotationReceipt({
  consumedTokenHash,
  replacementTokenHash,
}: DeleteReceiptOptions): Promise<boolean> {
