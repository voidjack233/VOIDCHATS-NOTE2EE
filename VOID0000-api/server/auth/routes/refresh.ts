...
interface ReplacementRefreshTokenOptions {
  receipt: RefreshRotationReceipt | null;
  tokenRecord: RefreshTokenRecord;
  decoded: import('../types.js').AuthTokenClaims;
}
...
const clearAuthCookies = (req: Request, res: Response): void => {
...
const getReplacementRefreshToken = ({
  receipt,
  tokenRecord,
  decoded,
}: ReplacementRefreshTokenOptions): string | null => {
  if (
    !receipt ||
  ...
  try {
    ...
    if (!isAuthTokenClaims(replacementDecoded, 'refresh')) return null;
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (!isAuthTokenClaims(decoded, 'refresh')) {
      return res.status(403).json({
        success: false,
        code: 'TOKEN_INVALID',
    ...
    if (result.rows.length === 0) {
      const predecessorResult = await client.query<RefreshTokenRecord>(
        `SELECT rt.*, u.is_verified
         FROM refresh_tokens rt
