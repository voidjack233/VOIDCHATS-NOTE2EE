...
type LoginSessionRecord = Awaited<ReturnType<typeof createLoginSessionRecord>>;
...
interface LoginDatabasePool extends DatabaseQueryable {
  connect(): Promise<PoolClient>;
}

interface PostClaimErrorBody {
  message: string;
  [key: string]: unknown;
}
...
interface VerifiedEmailSnapshotOptions {
  claimedSession: PendingTwoFactorSession | null;
  expectedSession: PendingTwoFactorSession | null;
  verifiedEmailCodeHash: string | null;
  now?: number;
}

interface VerifyLoginDependencies {
  databasePool?: LoginDatabasePool;
  claimChallenge?: typeof claimPendingTwoFactorSession;
  finalizeChallenge?: typeof finalizeClaimedTwoFactorSession;
  ...
  recordLoginSuccess?: (req: Request, userId: string) => Promise<unknown>;
  recordLoginTrust?: (req: Request, res: Response) => Promise<unknown>;
}

export { create2FASession };

function sendTwoFactorCounterUnavailable(res: Response) {
...
async function sendFailedTwoFactorAttempt(
  res: Response,
...
class PostClaimValidationError extends Error {
  ...
  readonly body: PostClaimErrorBody;

  constructor(status: number, body: PostClaimErrorBody) {
    super(body.message);
...
function sendFreshLoginRequired(res: Response, code: string) {
  return res.status(503).json({
    success: false,
...
function sendRetryableLoginCompletionError(res: Response) {
  return res.status(503).json({
    success: false,
...
async function validateTotpCandidate(
  queryable: DatabaseQueryable,
  userId: string,
  code: string,
): Promise<'unavailable' | 'valid' | 'invalid'> {
...
function isExactVerifiedEmailSnapshot({
  ...
  now = Date.now(),
}: VerifiedEmailSnapshotOptions): boolean {
...
}: VerifyLoginDependencies = {}) {
  return async function verifyLogin(req: Request, res: Response) {
    let dbClient: PoolClient | null = null;
    let transactionState: TransactionState = 'not_started';
    const getTransactionState = (): TransactionState => transactionState;
    let claimOwnerId: string | null = null;
    ...
    try {
      ...
      if (method === 'backup') {
        const backupCodes = await databasePool.query<BackupCodeRow>(
          `SELECT id, code_hash
        ...
        if (!session.emailCodeHash || Date.now() > Number(session.emailCodeExpiresAt)) {
          return res.status(400).json({
            success: false,
      ...
      if (method === 'totp') {
        ...
        method === 'backup' &&
        (backupCodeId === null || !await consumeBackupCode(dbClient, backupCodeId, userId))
      ) {
        throw new PostClaimValidationError(400, {
          success: false,
      ...
      const failedTransactionState = getTransactionState();
      if (
        failedTransactionState === 'committed' ||
        failedTransactionState === 'commit_uncertain'
      ) {
      ...
      if (failedTransactionState === 'beginning') {
        destroyDbClient = true;
      }

      const rollbackConfirmed = failedTransactionState === 'open'
        ? await rollbackOpenTransaction()
        : failedTransactionState !== 'rollback_failed';
