...
interface IpStatsRow extends QueryResultRow {
...
export class IPSecurity {
  static async logIPActivity(
    req: Request,
    action: string,
    userId: string | null = null,
    client: DatabaseQueryable | null = null,
  ): Promise<boolean> {
    try {
  ...
  static async hasExcessiveFailures(
    ip: string,
    threshold: number = 10,
    client: DatabaseQueryable | null = null,
  ): Promise<boolean> {
    try {
      ...
      if (client) {
        const result = await client.query<FailureCountRow>(query, [ip]);
        return Number.parseInt(String(result.rows[0]?.failure_count || 0), 10) >= threshold;
      } else {
        const result = await pool.query<FailureCountRow>(query, [ip]);
        return Number.parseInt(String(result.rows[0]?.failure_count || 0), 10) >= threshold;
      }
    } catch (err) {
  ...
  static async hasExcessiveDeviceFailures(
    fingerprint: string,
    threshold: number = 5,
    client: DatabaseQueryable | null = null,
  ): Promise<boolean> {
    try {
      ...
      if (client) {
        const result = await client.query<FailureCountRow>(query, [fingerprint]);
        return Number.parseInt(String(result.rows[0]?.failure_count || 0), 10) >= threshold;
      } else {
        const result = await pool.query<FailureCountRow>(query, [fingerprint]);
        return Number.parseInt(String(result.rows[0]?.failure_count || 0), 10) >= threshold;
      }
    } catch (err) {
  ...
  static isSuspiciousIP(ip: string | null | undefined): boolean {
    if (!ip) return true;
  ...
  static async getIPStats(
    ...
    try {
      ...
      if (client) {
        const result = await client.query<IpStatsRow>(query, [ip]);
        return result.rows[0] || null;
      } else {
        const result = await pool.query<IpStatsRow>(query, [ip]);
        return result.rows[0] || null;
      }
    } catch (err) {
...
export class DeviceManager {
   ...
   */
  static generateDeviceId(req: Request, res: Response | null = null): string {
    if (res) {
  ...
  static getDeviceInfo(req: Request): DeviceInfo {
    ...
    const requestedDeviceName =
      ...
      req.body !== null &&
      'deviceName' in req.body &&
      typeof req.body.deviceName === 'string'
        ? req.body.deviceName
        : '';
    if (requestedDeviceName.trim()) {
      const customName = requestedDeviceName.trim();
