...
type QueueImageUploadInput = Omit<ImageJobData, 'queuedAt'>;

let sharpModulePromise: Promise<SharpFactory> | undefined;

function getSharp(): Promise<SharpFactory> {
  if (!sharpModulePromise) {
    sharpModulePromise = import('sharp').then((module) => module.default as SharpFactory);
  }
...
export const imageQueue = new Queue<ImageJobData, ImageJobResult, 'process-avatar'>(
  'image-processing',
  { connection },
);
...
export async function queueImageUpload({
  ...
  imageData,
  oldFilename,
}: QueueImageUploadInput): Promise<{
  jobId: string | undefined;
  job: Job<ImageJobData, ImageJobResult, 'process-avatar'>;
  status: 'queued';
}> {
...
export function startImageWorker(): Worker<ImageJobData, ImageJobResult, 'process-avatar'> {
  const worker = new Worker<ImageJobData, ImageJobResult, 'process-avatar'>(
    'image-processing',
    async (job) => {
      ...
      if (oldFilename) {
        try {
          ...
          console.warn(`⚠️ Could not delete old avatar: ${message}`);
        }
      }
