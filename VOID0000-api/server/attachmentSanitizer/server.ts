...
import {
  ...
  type SanitizedChatAttachmentImage,
...
type SanitizationResult = SanitizedChatAttachmentImage | null;
type SanitizerTask = () => SanitizationResult | PromiseLike<SanitizationResult>;

type QueueItem = {
  reservation: QueueReservation;
  task: SanitizerTask;
  resolve: (result: SanitizationResult) => void;
...
export type AttachmentSanitizerServerOptions = {
  socketPath?: string;
  concurrency?: unknown;
  queueDepth?: unknown;
  ...
  sanitize?: (
    source: Buffer,
    claimedMime: string,
  ) => SanitizationResult | PromiseLike<SanitizationResult>;
  runSharpWork?: SharpWorkRunner;
};

export interface AttachmentSanitizerServer {
  socketPath: string;
  getStats(): {
    active: number;
    queued: number;
...
function capacityError(): AttachmentSanitizerTransportError {
...
function shutdownError(): AttachmentSanitizerTransportError {
  return new AttachmentSanitizerTransportError(
...
class BoundedAttachmentWorkQueue {
  ...
  reserve(payloadLength: number): QueueReservation {
    ...
    const reservation: QueueReservation = {
      payloadLength,
      state: 'reserved',
    };
  ...
  submit(
    ...
    return new Promise<SanitizationResult>((resolve, reject) => {
...
): asserts message is Record<string, unknown> & SanitizeRequest {
  if (
    ...
    throw new AttachmentSanitizerTransportError(
      ...
      {
        code: typeof message.payloadLength === 'number' &&
          message.payloadLength > MAX_ATTACHMENT_SANITIZER_PAYLOAD_BYTES
          ? 'ATTACHMENT_TOO_LARGE'
          : 'ATTACHMENT_SANITIZER_PROTOCOL_ERROR',
        status: typeof message.payloadLength === 'number' &&
          message.payloadLength > MAX_ATTACHMENT_SANITIZER_PAYLOAD_BYTES
          ? 413
          : 400,
        retryable: false,
...
export async function startAttachmentSanitizerServer(
  options: AttachmentSanitizerServerOptions = {},
): Promise<AttachmentSanitizerServer> {
