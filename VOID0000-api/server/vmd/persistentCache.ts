...
import type { VmdImageVariant } from './capability.js';
import type { VmdTransformedImage } from './imageVariants.js';
...
export type VmdVariantIdentity = {
...
export type VmdCachedImage = VmdTransformedImage & { etag: string };

export type VmdCacheReadResult =
  | { status: 'hit'; image: VmdCachedImage }
  | { status: 'miss' }
  | { status: 'unavailable'; error: unknown }
  | { status: 'corrupt'; error?: unknown };

export interface VmdPersistentCache {
  read(identity: VmdVariantIdentity): Promise<VmdCacheReadResult>;
  write(identity: VmdVariantIdentity, image: VmdTransformedImage): Promise<string>;
}
...
function sha256Hex(value: BinaryLike): string {
...
function parsePositiveMetadata(value: unknown): number | null {
  const parsed = Number(value);
...
export function createVmdVariantIdentity({
  objectKey,
  objectStat,
  variant,
}: {
  objectKey: unknown;
  objectStat: Partial<BucketItemStat> | null | undefined;
  variant: VmdImageVariant;
}): VmdVariantIdentity {
...
}: { client?: Client; bucket?: string } = {}): VmdPersistentCache {
  return {
    async read(identity: VmdVariantIdentity): Promise<VmdCacheReadResult> {
    ...
    async write(
      identity: VmdVariantIdentity,
      image: VmdTransformedImage,
    ): Promise<string> {
...
export function initializeVmdCacheStorage(): Promise<{
  bucket: string;
  retentionDays: number;
}> {
