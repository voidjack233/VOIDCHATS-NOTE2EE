...
export type VmdRenderedImage = VmdTransformedImage & { etag?: string };

interface AttachmentObjectRow extends QueryResultRow {
  object_key: string;
}
...
const transformQueue = new VmdWorkQueue<VmdRenderedImage>({
...
const lastCacheWarningAt = new Map<string, number>();

function warnCacheFailure(kind: string, error: unknown): void {
  const now = Date.now();
...
async function findAttachmentObject(
  ...
  const result = await pool.query<AttachmentObjectRow>(
...
async function renderStoredImage(
  attachmentId: string,
  ...
  let generated: VmdRenderedImage;
  try {
    ...
    if (error instanceof VmdMediaError && error.code === 'VMD_AT_CAPACITY') {
      incrementVmdMetric('queue_full');
    } else if (error instanceof VmdMediaError && error.code === 'VMD_QUEUE_TIMEOUT') {
...
export function renderStoredVmdImage(
  attachmentId: string,
  variant: VmdImageVariant,
): Promise<VmdRenderedImage> {
