...
export type VmdImageVariant = keyof typeof VMD_IMAGE_VARIANTS;
export type VmdResponsiveImageVariant = typeof VMD_RESPONSIVE_IMAGE_VARIANTS[number];
type SigningKey = BinaryLike | KeyObject;

export type VmdCapabilityOptions = {
  now?: number;
  publicOrigin?: string;
  signingKey?: SigningKey;
  ttlSeconds?: number;
  expiryBucketSeconds?: number;
};

export type VmdImageDelivery = {
  display_url: string;
  display_url_expires_at: number;
};

type VmdVariantDelivery = {
  url: string;
  expires_at: number;
  width: number;
};
...
function resolveCapabilityTtlSeconds(
  value: unknown = process.env.VMD_SIGNED_URL_TTL_SECONDS,
): number {
...
function resolveCapabilityExpiresAt(
  now: number,
  ttlSeconds: number,
...
function resolvePublicOrigin(value: string | undefined = process.env.VMD_PUBLIC_URL): string {
...
export function getVmdSigningKey(): Buffer {
...
export function isVmdImageVariant(value: unknown): value is VmdImageVariant {
...
export function createVmdImageDelivery(
  attachmentId: string,
  variant: VmdImageVariant = 'medium',
  {
    now = Date.now(),
...
export function createVmdResponsiveImageDelivery(
  ...
  return {
    ...
    display_variants: {
      small: {
        url: small.display_url,
        expires_at: small.display_url_expires_at,
        width: VMD_IMAGE_VARIANTS.small.bound,
      },
      medium: {
        url: medium.display_url,
        expires_at: medium.display_url_expires_at,
        width: VMD_IMAGE_VARIANTS.medium.bound,
      },
      large: {
        url: large.display_url,
        expires_at: large.display_url_expires_at,
        width: VMD_IMAGE_VARIANTS.large.bound,
      },
    },
  };
}
