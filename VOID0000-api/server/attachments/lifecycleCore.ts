...
type QueryResultLike<Row extends LifecycleRow = LifecycleRow> = {
  rows: Row[];
  rowCount: number | null;
};

interface LifecycleQueryable {
  query<Row extends LifecycleRow = LifecycleRow>(
    sql: string,
    params?: unknown[],
  ): Promise<QueryResultLike<Row>>;
}

interface LifecycleClient extends LifecycleQueryable {
  release(): void;
}

interface LifecycleDbPool extends LifecycleQueryable {
...
interface AttachmentObjectStore {
  ...
  statObject(bucket: string, objectKey: string): Promise<ObjectListing>;
  listObjectsV2(
    bucket: string,
    prefix: string,
    recursive: boolean,
...
export type AttachmentLifecycleConfig = Readonly<{
  stagedTtlSeconds: number;
  reservationTtlSeconds: number;
  stagedMaxCount: number;
...
type PreparedAttachment = Omit<StagedAttachmentInput, 'buffer'> & {
  buffer: Buffer;
  contentHash: string;
  objectKey: string;
  sizeBytes: number;
...
type StageUploadedAttachmentsInput = {
  userId: string;
  conversationId: string;
  attachments: StagedAttachmentInput[];
};

type DeleteStagedAttachmentInput = {
  attachmentId: string;
  userId: string;
  conversationId: string;
};

type ReservationRow = LifecycleRow;

export type AttachmentReservationClassification =
  | { state: 'staged'; messageId: null }
  | { state: 'committed'; messageId: string }
  | { state: 'reserved'; messageId: string; reservationExpired: boolean };

export type AttachmentReservationResult =
  | (ReservationIdentity & { state: 'none' })
  | (ReservationIdentity & { state: 'reserved_new' })
  | (ReservationIdentity & { state: 'committed' })
  | (ReservationIdentity & { state: 'reserved'; reservationExpired: boolean });

type AttachmentLifecycleOptions = {
  dbPool?: LifecycleDbPool;
  objectStore?: AttachmentObjectStore;
  bucket?: string;
  config?: AttachmentLifecycleConfig;
  logger?: LifecycleLogger;
};

type StagedUploadQuota = {
  currentCount: number;
  currentBytes: number;
  incomingCount: number;
  incomingBytes: number;
...
function resolvePositiveInteger(value: unknown, fallback: number, maximum: number): number {
...
export function resolveAttachmentLifecycleConfig(
  env: Record<string, string | undefined> = process.env,
): AttachmentLifecycleConfig {
...
export function createAttachmentContentHash(buffer: unknown): string {
...
export function createAttachmentBlobObjectKey(contentHash: unknown): string {
...
export function parseAttachmentBlobObjectKey(objectKey: unknown): string | null {
...
function isOlderThanGrace(lastModified: unknown, graceSeconds: number): boolean {
...
function parseAttachmentDescriptor(rawAttachment: unknown): {
  id: string;
  stableUrl: string;
} {
  ...
  let descriptor: Record<string, unknown> = { url: raw };
...
export function extractProtectedAttachmentIds(rawAttachments: unknown): string[] {
...
function normalizeRowId(row: Pick<LifecycleRow, 'id'>): string {
...
export function classifyAttachmentReservation({
  ...
  conversationId: string;
  reservationId: string;
}): AttachmentReservationClassification {
...
async function readStagedUsage(
  queryable: LifecycleQueryable,
  userId: string,
...
export function createAttachmentLifecycle({
  ...
  async function assertUploadAllowed({
    ...
    incomingCount,
    incomingBytes,
  }: UploadAllowance): Promise<void> {
    const usage = await readStagedUsage(activeDbPool, userId);
  ...
  async function stageUploadedAttachments({
    ...
    conversationId,
    attachments,
  }: StageUploadedAttachmentsInput) {
    const preparedAttachments: PreparedAttachment[] = attachments.map((attachment) => {
      const buffer = attachment.buffer;
      if (!Buffer.isBuffer(buffer) || buffer.length === 0) {
        throw new TypeError('Staged attachments require final non-empty bytes');
      }
      const contentHash = createAttachmentContentHash(buffer);
      return {
        ...attachment,
        buffer,
        contentHash,
    ...
    return withTransaction(activeDbPool, async (client) => {
      ...
      for (const contentHash of [...attachmentGroups.keys()].sort()) {
        ...
        await activeObjectStore.putObject(
          activeBucket,
  ...
  async function deleteStagedAttachment({
    attachmentId,
    userId,
    conversationId,
  }: DeleteStagedAttachmentInput) {
    return withTransaction(activeDbPool, async (client) => {
  ...
  async function releaseReservation({
    ...
    userId,
    conversationId,
  }: ReservationIdentity): Promise<number | null> {
    if (attachmentIds.length === 0) {
      return 0;
    }

    const result = await activeDbPool.query(
      `UPDATE attachment_objects
       SET status = 'staged',
           reserved_at = NULL,
           reserved_until = NULL,
  ...
  async function cleanupExpiredStaged({
    batchSize = config.cleanupBatchSize,
  }: { batchSize?: number } = {}) {
    const candidateResult = await activeDbPool.query(
      `SELECT id
       FROM attachment_objects
       WHERE status = 'staged'
         AND expires_at <= NOW()
    ...
    for (const candidate of candidateResult.rows) {
      try {
        const result = await withTransaction(activeDbPool, async (client) => {
          const lockedResult = await client.query(
            `SELECT id
             FROM attachment_objects
             WHERE id = $1
  ...
  async function cleanupOrphanedBlobs({
    ...
    for (const candidate of candidateResult.rows) {
      let blob: { id: string; bucket: string; objectKey: string } | null = null;
      try {
        blob = await withTransaction(activeDbPool, async (client) => {
          const lockedResult = await client.query(
        ...
        await activeObjectStore.removeObject(activeBlob.bucket, activeBlob.objectKey);
        const deleteResult = await withTransaction(activeDbPool, async (client) => (
          client.query(
  ...
  async function cleanupUntrackedContentAddressedObjects({
    ...
    for (const candidate of candidates) {
      ...
      try {
        const outcome = await withTransaction(activeDbPool, async (client) => {
          await client.query(
            'SELECT pg_advisory_xact_lock(hashtext($1))',
            [`attachment-blob:${contentHash}`],
          );
          ...
          const blobResult = await client.query(
            `SELECT id, bucket, object_key, content_hash, status
             ...
             WHERE (bucket = $1 AND object_key = $2)
                OR content_hash = $3
             LIMIT 2`,
            [activeBucket, objectKey, contentHash],
          );
          if (blobResult.rows.length > 0) {
            const exact = blobResult.rows.find((row) => (
              row.bucket === activeBucket && row.object_key === objectKey
            ));
          ...
          let currentObject;
          try {
            currentObject = await activeObjectStore.statObject(activeBucket, objectKey);
          } catch (error) {
          ...
          await activeObjectStore.removeObject(activeBucket, objectKey);
...
export function createAttachmentReservationId(clientMessageId: unknown): string {
