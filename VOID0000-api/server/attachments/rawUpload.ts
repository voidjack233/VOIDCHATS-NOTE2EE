...
export interface AttachmentRawUploadLimits {
  maxFileBytes: number;
  maxFilenameHeaderBytes: number;
  maxFilenameCharacters: number;
  maxMimeHeaderBytes: number;
  maxBlurhashHeaderBytes: number;
  maxDimension: number;
}

export interface AttachmentClientMetadata {
  name?: string;
  mime: string;
  width?: number;
  height?: number;
...
export interface ParsedAttachmentRawRequest {
  file: {
    buffer: Buffer;
    clientFilename: string;
    clientMimeType: string;
    metadata: AttachmentClientMetadata;
  };
}

interface ParseAttachmentRawRequestOptions {
  limits?: Partial<AttachmentRawUploadLimits>;
}
...
function resolveLimits(
  overrides: Partial<AttachmentRawUploadLimits> = {},
): AttachmentRawUploadLimits {
  const resolved = {
...
function readHeader(request: Request, name: string, maximumBytes: number): string {
...
function readFilename(
  request: Request,
  limits: AttachmentRawUploadLimits,
): string | undefined {
...
function readClaimedMime(request: Request, limits: AttachmentRawUploadLimits): string {
...
function assertBinaryContentType(request: Request): void {
...
export async function parseAttachmentRawRequest(
  request: Request,
  options: ParseAttachmentRawRequestOptions = {},
): Promise<ParsedAttachmentRawRequest> {
  ...
  let metadata: Omit<AttachmentClientMetadata, 'size'> & { size?: number };
  try {
    metadata = {
  ...
  const buffer = await new Promise<Buffer>((resolve, reject) => {
    ...
    const rejectOnce = (error: AttachmentRawUploadError): void => {
      if (settled) return;
      settled = true;
      cleanup();
      request.resume();
  ...
  const completedMetadata: AttachmentClientMetadata = {
    ...metadata,
    size: buffer.length,
  };
  return {
