 ...
 */
export function initSubscriber(handler: (message: unknown) => void): Redis {
 ...
 */
export function publishToGateway(
  event: string,
  targetUserId: string,
  data: unknown,
): void {
...
export function publishGatewayCommand(command: string, data: unknown): void {
  if (!publisher) {
